 ...
 * `?categoryId=` / `?productId=` / `?productTypeId=` / `?productVariantId=`
 * narrow the feed to one level of the catalogue.
 */
export async function GET(request: NextRequest) {
  ...
  const productTypeId = params.get("productTypeId") || undefined;
  const productVariantId = params.get("productVariantId") || undefined;
  const feed = await getGalleryFeedPage(page, {
    categoryId,
    productId,
    productTypeId,
    productVariantId,
